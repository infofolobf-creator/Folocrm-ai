              ...
              {alerts.length === 0 ? (
                ...
                alerts.map(alert => (
                  <div key={alert.id} className="p-4 rounded-xl bg-slate-50 border border-slate-100 space-y-2 hover:border-emerald-200 transition">
                    ...
                    <p className="text-slate-500 text-xs line-clamp-2">{alert.summary}</p>
                    <div className="bg-emerald-50/50 p-2.5 rounded-lg border border-emerald-100/30 text-[11px] text-slate-700">
                      <span className="font-semibold text-emerald-800">Impact FOLO :</span> {alert.impactOnFolo}
                    </div>
                    {alert.url && (
                      <a href={alert.url} target="_blank" rel="noopener noreferrer" className="text-[11px] text-emerald-600 hover:underline inline-block">
